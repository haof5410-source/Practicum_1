...
今天上午我把下午要答辩的文档和代码，sql进行了调优然后下午我就数据导入到doris然后报错到不进去，然后我就将wsl上安装了es然后画图没时间了


### 2025-11-05

今天把es的图表完成后，和把导入到doris是的报错修改好了

### 2025-11-06

然后今天我在fineReport上安装了一个桑基图然后然后将访问路径做成桑基图
...
1. 基于模型生成用户的评论数据，分为好评和差评
2. 数据写入sqlserver

### 2025-11-13

今天将SQLSevere的用户评论敏感词分级
...
今天完成的功能是就爱那个SQLSevere的评论数据进行分层

### 2025-11-18

今天我把SQLSevere准备把数据同步到hbase上，但是我的hbase损坏了